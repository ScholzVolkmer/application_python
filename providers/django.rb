...
action :before_compile do
  ...
  new_resource.migration_command "#{::File.join(new_resource.virtualenv, "bin", "python")} #{::File.join(new_resource.project_name, "manage.py")} syncdb --noinput" if !new_resource.migration_command
...
action :before_symlink do
  ...
  if new_resource.collectstatic
    ...
    execute "#{::File.join(new_resource.virtualenv, "bin", "python")} #{::File.join(new_resource.project_name, "manage.py")} #{cmd}" do
      user new_resource.owner
      group new_resource.group
      cwd new_resource.release_path
    end
